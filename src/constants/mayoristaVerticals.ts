...
interface MayoristaVerticalBase {
  slug: string
  badge: string
  h1: string
  description: string
  ...
  puntos: string[]
  /** Mensaje base del botón de WhatsApp — el número se resuelve en getMayoristaVerticals(). */
  whatsappMsg: string
  /** Las respuestas usan {{MIN}} como placeholder del mínimo de compra (viene de /admin/zonas). */
  faqs: FaqItem[]
}

export interface MayoristaVertical extends Omit<MayoristaVerticalBase, 'whatsappMsg' | 'faqs'> {
  whatsappUrl: string
  faqs: FaqItem[]
}

const MAYORISTA_VERTICALS_BASE: MayoristaVerticalBase[] = [
  {
    ...
    whatsappMsg: 'Hola! Tengo un kiosco y quiero consultar precios mayoristas de bebidas 🧃',
    faqs: [
      ...
      { id: 'kioscos-2', q: '¿Cuál es el pedido mínimo para kioscos?', a: 'Como con cualquier pedido mayorista, el mínimo es de {{MIN}}.' },
  ...
  },
  {
    ...
    whatsappMsg: 'Hola! Tengo un almacén y quiero consultar precios mayoristas de bebidas 🛒',
    faqs: [
      ...
      { id: 'almacenes-2', q: '¿Cuál es el mínimo de compra mayorista?', a: 'El mínimo para acceder a precio mayorista es de {{MIN}} por pedido.' },
  ...
  },
  {
    ...
    whatsappMsg: 'Hola! Tengo un restaurante/bar y quiero consultar precios mayoristas de bebidas 🍺',
    faqs: [
      ...
      { id: 'restaurantes-3', q: '¿Cuál es el pedido mínimo?', a: 'El mínimo para precio mayorista es de {{MIN}} por pedido.' },
  ...
  },
  {
    ...
    whatsappMsg: 'Hola! Estoy organizando un evento y quiero consultar precios mayoristas de bebidas 🎉',
    faqs: [
      ...
      { id: 'eventos-2', q: '¿Hay un pedido mínimo para eventos?', a: 'Se aplica el mismo mínimo mayorista de {{MIN}} por pedido.' },
...
/**
 * Verticales mayoristas con el mínimo de compra (/admin/zonas) y el número de
 * WhatsApp (/admin/config) ya resueltos. Antes esto se calculaba una sola vez al
 * cargar el módulo, con el mínimo y el número fijos del código.
 */
export function getMayoristaVerticals(minimoMonto: string = MAYORISTA_MIN_MONTO, numero?: string): MayoristaVertical[] {
  return MAYORISTA_VERTICALS_BASE.map(({ whatsappMsg, faqs, ...rest }) => ({
    ...rest,
    whatsappUrl: buildWhatsAppUrl(whatsappMsg, numero),
    faqs: faqs.map((f) => ({ ...f, a: f.a.replaceAll('{{MIN}}', minimoMonto) })),
  }))
}
